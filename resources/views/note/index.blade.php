            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($notes as $note)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap h-10">
                            <div class="note-buttons flex">
                                <form action="{{ route('note.copy', $note) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-700">Kopiuj</button>
                                </form>
                                <a href="{{ route('note.show', $note) }}"
                                    class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-700">Wyświetl</a>
                                <a href="{{ route('note.edit', $note) }}"
                                    class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-700">Edytuj</a>
                                <form action="{{ route('note.destroy', $note) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-700">Usuń</button>
                                </form>
                            </div>
                        </td>
                    </tr>
